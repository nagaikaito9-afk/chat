...
function setupPaintModal() {
  ...
  opacityPicker.addEventListener('input', updateBrushPreview);

  // Background Mode Toggle
  bgTypeSelect.addEventListener('change', (e) => {
    const bgMode = e.target.value;
    canvasWrapper.className = `paint-canvas-wrapper bg-${bgMode}`;
  });

  // Open Modal Listener
  openBtn.addEventListener('click', () => {
    canvasWrapper.className = `paint-canvas-wrapper bg-${bgTypeSelect.value}`;
    initCanvas();
    updateBrushPreview();
  ...
  // Get Relative Mouse / Touch Position
  function getPos(e) {
  ...
  const startDraw = (e) => {
    ...
    if (currentTool === 'eyedropper') {
      ...
      document.querySelector('.paint-tool-btn[data-tool="pen"]').click();
      isDrawing = false;
      return;
    }
  ...
  // Helper: Export Canvas with Background
  function getExportDataUrl() {
    const bgMode = bgTypeSelect.value;
